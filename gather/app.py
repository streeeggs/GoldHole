...
def isItCloseToMidnight():
    if datetime.today() + timedelta(minutes=2) > datetime.combine(
...
def isItCloseToMidnightOrToKill():
    return isItTimeToKill() or isItCloseToMidnight()
...
def addFile(result, fileName, path):
    ...
    if exists(os.path.join(path, str(fileName))):
        with open(
            os.path.join(path, str(fileName)), "r", encoding="utf8", errors="ignore"
        ) as old_json:
    ...
    with open(
        os.path.join(path, str(fileName)), "w", encoding="utf8", errors="ignore"
    ) as json_file:
...
def parseUserMessage(html):
    ...
    try:
        ...
        # Get username
        username_class = re.search(
            'class\="chat-msg-[\-\w\u00c0-\u00ff]{1,20}["|\s]', html
        ).group(
            0
        )  # class="chat-msg-user_name"
        ...
        timestamp = "{0} {1}".format(
            str(date.today()),
            re.search("\[\d{2}\:\d{2}\:\d{2}\]", html).group(0).strip("[").strip("]"),
        ...
        message = data_text.split("=")[1].split('"')[
            1
        ]  # Alternatively, I could have removed data-text= and > then i could just strip('"')
...
def get_gold(lottery_js):
    ...
    try:
        ...
        timestamp_gold = page.locator(
            "#messagebuffer div:has(.text-lottery) >> .timestamp"
        )
        message_gold = page.locator(
            "#messagebuffer div:has(.text-lottery) >> .text-lottery"
        )
    ...
    while not title_text_origin:
        title_text_origin = title.all_inner_texts()
        if isItCloseToMidnightOrToKill():
            break

    title_text_origin = title_text_origin[0]
    title_text_current = title_text_origin

    # While the title_text_current isn't an empty string and matches the origin. Or break if it's close to midnight
    while title_text_current and title_text_current == title_text_origin:
        if isItCloseToMidnightOrToKill():
            break
        ...
        user_array = [
            {
                "title": gold[0],
                "name": gold[2],
                "message": {"time": gold[1], "text": gold[3]},
            }
            for gold in unique_golds_by_user
        ]
        video_array = [
            {"title": gold[0], "message": {"time": gold[1], "text": gold[2]}}
            for gold in unique_golds_global
        ]
